...
class instructions():
    ...
    def instructions_initial_info(self, s: requests.session, request_body: dict):
        return s.get(f"{base_api_url}/{self.endpoint}")

    def instructions_refactor(s: requests.session, query_params:dict, request_body: dict):
        endpoint = "/instructions"
        query = {"carBrandId": query_params.get("carBrandId", 0),
                 "carModelId": query_params.get("carModelId", 0),
